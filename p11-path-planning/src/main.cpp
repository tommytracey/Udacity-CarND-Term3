...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane,&ref_vel](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      // leverage the previous path if one exists
      ...
      // Trajectory: calculate the car's path

			// list of (x,y) waypoints spaced at 30m
			vector<double> ptsx;
			vector<double> ptsy;
			...
			// if no previous path, use the car as starting reference
			if (prev_size < 2) {
				// make path tangent to the car using most recent telemetry
				...
				// use the previous path's endpoints
			...
			// define the (x,y) points we will use
			vector<double> next_x_vals;
			vector<double> next_y_vals;

			// start by grabbing the previous path points
			for (int i = 0; i < previous_path_x.size(); i++) {
				next_x_vals.push_back(previous_path_x[i]);
				next_y_vals.push_back(previous_path_y[i]);
			}

			// compute y points and distance so we can smoothly maintain our ref velocity
